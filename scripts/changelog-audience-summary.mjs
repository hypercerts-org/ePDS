...
function slugify(text) {
  ...
  const slice = raw.slice(0, SLUG_MAX_LEN)
...
console.log(
  `changelog-audience-summary: added "Who should read this release" block with ${sortedAudiences.length} audience(s) and ${entries.length} anchored entrie(s) to ${releaseHeading.trim()}`,
)
