...
| `EPDS_CALLBACK_SECRET`     | **Required.** HMAC-SHA256 secret signing the `/oauth/epds-callback` redirect — generate with `openssl rand -hex 32`. Both pds-core and auth-service refuse to start if unset.                                                                                               |
...
| `AUTH_SESSION_SECRET` | **Required.** Session secret — generate with `openssl rand -hex 32`. Auth service refuses to start if unset.                                                                                                |
